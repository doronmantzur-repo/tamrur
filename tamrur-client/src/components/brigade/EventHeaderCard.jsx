// External libraries
import { Badge, Box, Group, Menu, Stack, Text } from "@mantine/core";
...
/**
 * Shared chrome for every header badge, merged into each one's own colors:
 * a fixed height matching the top bar's open/close-event buttons (2.5rem),
 * so the two rows read as the same scale, plus the hover feedback.
 */
...
/**
 * Renders the brigade event dashboard's header: the type/status/aerial-evac/
 * gathering/evac-status badges, centered across the card's full width. The
 * event name and its client-only note live in the top bar instead (see
 * `EventNameBlock`, rendered under "לוח בקרה: חטיבה" in
 * `EventDashboardPage`), not here. The status badge is itself the control:
 * click it to open a menu of every non-completed status and set the event
 * to any of them directly, since progress here isn't strictly linear —
 * except "completed" is final: once the event is closed, the badge turns
 * into a plain, non-clickable label instead of a menu, since closing is a
 * one-way action the brigade can no longer undo from here (per team
 * decision, 2026-08-16). Casualty/evacuation counts, the elapsed-time chip,
 ...
 */
const EventHeaderCard = ({ event, aerialEvacStatus, onStatusChange }) => {
  ...
  return (
    <DashboardCard
      padding="sm"
      gap={2}
      // No title/titleContent — the event name moved to the top bar
      // (EventNameBlock). With no title sibling to share the row with, the
      // headerExtra Box below still fills the row and centers the badges,
      // now across the card's full width.
      titleContent={<></>}
      headerExtra={
