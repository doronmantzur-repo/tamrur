 ...
 * to each casualty's ai_evacuation_priority column — deliberately not the
 * medic's own evac-priority, which stays hand-edited in the casualty table; the result list below
...
const EvacPriorityCard = ({ eventId }) => {
  ...
  const sortedCasualties = casualties
    ? [...casualties].sort(
        (a, b) => (a.ai_evacuation_priority ?? Infinity) - (b.ai_evacuation_priority ?? Infinity),
      )
    : [];
